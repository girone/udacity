...
selection1 <- subset(mtcars, mpg >= 30)
selection2 <- subset(mtcars,  hp < 60)
selection3 <- subset(mtcars, mpg >= 30 | hp < 60)
data <- mtcars[mtcars$mpg >= 30 & mtcars$hp < 60,]

states <- read.csv("stateData.csv")

statesWithN <- subset(states, state.abb == "ND")

###########

# Ordered Factors
setwd("/home/sternis/workspace/udacity/lessons/05 - Exploratory Data Analysis")
...
# Sort the levels of a factor (categorized) variable so that histograms become more readable.
...
levels(reddit$income.range)  # take the values, order them and reuse:
reddit$income.range <- factor(reddit$income.range, 
                              levels = c("Under $20,000",
                                         ...
                                         "$100,000 - $149,999",
                                         "$150,000 or more"), 
                              ordered = T)  # a different way to say `ordered()``
qplot(reddit$income.range)
